/********************************************************
 *
 * $GAMGI/src/gtk/molecule/gamgi_gtk_molecule_measure.c
 *
...
vbox_symmetry = (GtkWidget *) g_object_get_data (G_OBJECT (dialog), "symmetry_vbox_page");
...
if (row == 1)
  ...
  gtk_widget_hide (vbox_voronoi2);
  gtk_widget_hide (vbox_voronoi3);
  gamgi_gtk_measure_symmetry_init (window);
  }
...
if (row == 1) gamgi_gtk_measure_symmetry_ok (object, window);
...
/****************** external function ******************
 *                                                     *
 *           GAMGI_GTK_MOLECULE_MEASURE_PRESS          *
 *                                                     *
 *                                                     *
 *******************************************************/

void gamgi_gtk_molecule_measure_press (gamgi_window *window_mouse,
...
GAMGI_ENGINE_MOLECULE, FALSE, static_press);
}

/****************** external function ******************
 *                                                     *
 *              GAMGI_GTK_MOLECULE_MEASURE             *
 *                                                     *
 *                                                     *
 *******************************************************/

void gamgi_gtk_molecule_measure (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Molecule Measure", window);
window->action = GAMGI_GTK_MOLECULE_MEASURE;
...
gtk_list_store_append (store, &iter);
gtk_list_store_set (store, &iter, 0, "Symmetry", -1);
gtk_list_store_append (store, &iter);
...
/************************
 * insert Symmetry page *
 ************************/

gamgi_gtk_measure_symmetry (notebook, window);
