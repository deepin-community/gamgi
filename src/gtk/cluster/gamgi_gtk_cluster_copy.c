/***************************************************
 *
 * $GAMGI/src/gtk/cluster/gamgi_gtk_cluster_copy.c
 *
...
/**********************************************
 * when entries are empty, look for a cluster *
 **********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_CLUSTER;
...
/***********************************************
 * initialize cluster, name of copies, number  *
...
static gamgi_bool static_apply (gamgi_cluster *cluster, gamgi_window *window)
{
gamgi_layer *layer;
gamgi_cluster *cluster_new = NULL;
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT cluster);
else
  layer = cache.layer;
...
atom_start = gamgi_engine_dlist_atom_object (GAMGI_CAST_OBJECT cluster, NULL);
...
for (i = 0; i < cache.number; i++)
  {
  cluster_new = gamgi_engine_copy_cluster (cluster);
  if (cache.name[0] != '\0')
    strcpy (cluster_new->object.name, cache.name);

  gamgi_engine_link_cluster_layer (cluster_new, cache.layer);
  ...
  gamgi_mesa_translate_cluster (cluster_new, translate);
  }
...
if (gamgi->clusters == NULL)
  ...
  "Empty list of cluster objects", window_dialog); return; }
...
/******************************************
 * copy clusters according to dialog data *
 ******************************************/

start = NULL;
for (dlist = gamgi->clusters; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_CLUSTER dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *  any cluster object can be used here  *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->clusters->data;
...
"entry_cluster", GAMGI_ENGINE_CLUSTER, window_dialog);
if (cluster == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid cluster object", window_dialog);
  return;
  }
...
/*****************************************
 * copy cluster according to dialog data *
 *****************************************/

static_apply (cluster, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT cluster,
...
/*************** external function *************
 *                                             *
 *         GAMGI_GTK_CLUSTER_COPY_PRESS        *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_cluster_copy_press (gamgi_window *window_mouse,  
...
/*************** external function *************
 *                                             *
 *            GAMGI_GTK_CLUSTER_COPY           *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_cluster_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Cluster Copy", window);
window->action = GAMGI_GTK_CLUSTER_COPY;
