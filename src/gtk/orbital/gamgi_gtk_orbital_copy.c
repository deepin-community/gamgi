/*****************************************************
 *
 * $GAMGI/src/gtk/orbital/gamgi_gtk_orbital_copy.c
 *
...
/***********************************************
 * initialize orbital, name of copies, number  *
...
{
gamgi_layer *layer;
gamgi_orbital *orbital_new = NULL;
double translate[3];
int i;
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT orbital);
else
  layer = cache.layer;
...
for (i = 0; i < cache.number; i++)
  {
  orbital_new = gamgi_engine_copy_orbital (orbital);
  if (cache.name[0] != '\0')
    strcpy (orbital_new->object.name, cache.name);

  gamgi_engine_link_orbital_layer (orbital_new, cache.layer);

  gamgi_math_vector_add (translate, cache.shift, translate);
  gamgi_mesa_translate_orbital (orbital_new, translate);
  }
...
/******************************************
 * copy orbitals according to dialog data *
 ******************************************/

start = NULL;
for (dlist = gamgi->orbitals; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_ORBITAL dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *  any orbital object can be used here  *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->orbitals->data;
...
/*****************************************
 * copy orbital according to dialog data *
 *****************************************/

static_apply (orbital, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT orbital,
...
/***************** external function ****************
 *                                                  *
 *            GAMGI_GTK_ORBITAL_COPY_PRESS          *
 *                                                  *
 ****************************************************/

void gamgi_gtk_orbital_copy_press (gamgi_window *window_mouse, 
...
/**************** external function *****************
 *                                                  *
 *               GAMGI_GTK_ORBITAL_COPY             *
 *                                                  *
 *  Creates the dialog window used to copy orbitals *
 *                                                  *
 ****************************************************/

void gamgi_gtk_orbital_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Orbital Copy", window);
window->action = GAMGI_GTK_ORBITAL_COPY;
