/*****************************************************
 *
 * $GAMGI/src/gtk/orbital/gamgi_gtk_orbital_link.c
 *
...
/***********************************************
 * when entries are empty, look for an orbital *
 ***********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_ORBITAL;
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  if (row == 9) return GAMGI_ENGINE_BOND;
  }
else
  {
  combo = (GtkWidget *) g_object_get_data (G_OBJECT (dialog), "combo_below");
  row = gtk_combo_box_get_active (GTK_COMBO_BOX (combo));
  if (row == 0) return GAMGI_ENGINE_TEXT;
  }
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  {
  /**************************************************
   * the default for the object class below is Text *
   **************************************************/
 ...
 *                      STATIC_RESET                    *
 *                                                      *
 * Reset the orbital link dialog, cleaning all entries. *
 *                                                      *
 ********************************************************/
...
/**********************************
 * reset orbital and object names *
 **********************************/
...
/************************************************
 * when linking below reset only the object,    *
 * as the orbital might be useful for next task *
 ************************************************/
...
if (cache.child == TRUE)
  ...
  switch (parent->class)
    ...
    bond = GAMGI_CAST_BOND parent;
    gamgi_math_vector_zero (origin);
    if (bond->atom1 != NULL)
      gamgi_math_vector_add (origin, bond->atom1->position, origin);
    if (bond->atom2 != NULL)
      gamgi_math_vector_add (origin, bond->atom2->position, origin);
    gamgi_math_vector_scale (origin, origin, 0.5);
    break;

    case GAMGI_ENGINE_ATOM:
    atom = GAMGI_CAST_ATOM parent;
    gamgi_math_vector_copy (atom->position, origin);
    break;
...
/**************************
 * get child orbital list *
 **************************/

if (gamgi->orbitals == NULL)
  { gamgi_gtk_dialog_message_create ("Error",
  "Empty list of orbital objects", window_dialog); return; }
...
if (cache.above == TRUE)
  {
  /*******************************************
   * link above: link each listed orbital    *
  ...
  {
  /****************************************************
   * link below: link each listed orbital (parent) to *
...
"entry_orbital", GAMGI_ENGINE_ORBITAL, window_dialog);
if (orbital == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid orbital object", window_dialog);
  return;
  }
...
/*************************************************************************
 * link above: link selected orbital (child) to selected object (parent) *
 * link below: link selected orbital (parent) to selected object (child) *
 *************************************************************************/

if (cache.above == TRUE)
  { child = GAMGI_CAST_OBJECT orbital; parent = cache.object; }
else
  { child = cache.object; parent = GAMGI_CAST_OBJECT orbital; }
...
/***************** external function ****************
 *                                                  *
 *            GAMGI_GTK_ORBITAL_LINK_PRESS          *
 *                                                  *
 ****************************************************/

void gamgi_gtk_orbital_link_press (gamgi_window *window_mouse, 
...
/**************** external function *****************
 *                                                  *
 *              GAMGI_GTK_ORBITAL_LINK              *
 *                                                  *
 *  Creates the dialog window used to link orbitals *
 *                                                  *
 ****************************************************/

void gamgi_gtk_orbital_link (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Orbital Link", window);
window->action = GAMGI_GTK_ORBITAL_LINK;
