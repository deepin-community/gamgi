/*********************************************************
 *
 * $GAMGI/src/gtk/direction/gamgi_gtk_direction_select.c
 *
 ...
 * available only while the Direction->Select dialog exists. *
 *************************************************************/
...
gamgi_global_selection_link (gamgi->directions);
...
while (dlist != NULL)
  {
  gamgi_gtk_select_direction (store, NULL, GAMGI_CAST_DIRECTION dlist->data);
  dlist = dlist->next;
  }
...
switch (scope)
  {
  case STATIC_GAMGI:
  dlist = gamgi_engine_dlist_direction_gamgi (NULL);
  break;

  case STATIC_WINDOW:
  dlist = gamgi_engine_dlist_direction_window (window, NULL);
  break;

  case STATIC_LAYER:
  dlist = gamgi_engine_dlist_direction_layer (window->layer, NULL);
  break;
...
static_model (gamgi->directions, window);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  reference = 0;
  if (direction->reference == GAMGI_ENGINE_ATOM) reference = 1;
  condition = (reference >= min && reference <= max);

  if (condition != action)
    {
    dlist_new = gamgi_engine_dlist_add_start (dlist_new);
    dlist_new->data = direction;
    }
  }
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  model = -1;
  if (direction->model == GAMGI_PHYS_LINE) model = 0;
  if (direction->model == GAMGI_PHYS_POLE) model = 1;
  if (direction->model == GAMGI_PHYS_TRACE) model = 2;
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  if (direction->reference == GAMGI_ENGINE_CELL)
    condition = (direction->uvw[0] >= min && direction->uvw[0] <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  condition = FALSE;
  if (direction->reference == GAMGI_ENGINE_CELL)
    condition = (direction->uvw[1] >= min && direction->uvw[1] <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  condition = FALSE;
  if (direction->reference == GAMGI_ENGINE_CELL)
    condition = (direction->uvw[2] >= min && direction->uvw[2] <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  net = -1;
  if (direction->net == gamgi_phys_projection_wulff) net = 0;
  if (direction->net == gamgi_phys_projection_schmidt) net = 1;
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  if (direction->reference == GAMGI_ENGINE_CELL)
    for (i = min; i <= max; i++)
      if (gamgi_math_node_direction (direction, 0, i) == TRUE)
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  if (direction->reference == GAMGI_ENGINE_CELL)
    for (i = min; i <= max; i++)
      if (gamgi_math_node_direction (direction, 1, i) == TRUE)
        { condition = TRUE; break; }
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  if (direction->reference == GAMGI_ENGINE_CELL)
    for (i = min; i <= max; i++)
      if (gamgi_math_node_direction (direction, 2, i) == TRUE)
        { condition = TRUE; break; }
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  if (direction->reference == GAMGI_ENGINE_CELL)
    for (i = min; i <= max; i++)
      if (gamgi_math_node_direction (direction, 3, i) == TRUE)
        { condition = TRUE; break; }
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  direction = GAMGI_CAST_DIRECTION dlist->data;
  condition = (direction->red >= min && direction->red <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  direction = GAMGI_CAST_DIRECTION dlist->data;
  condition = (direction->green >= min && direction->green <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  direction = GAMGI_CAST_DIRECTION dlist->data;
  condition = (direction->blue >= min && direction->blue <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  direction = GAMGI_CAST_DIRECTION dlist->data;
  condition = (direction->object.scale >= min && direction->object.scale <= max);
...
/*****************************************************************
 * build list of candidate directions, according to chosen scope *
 *                                                               *
 * build list of selected directions, according to chosen method *
 *****************************************************************/
...
if (strcmp (name, "Pick") == 0)
  ...
  dlist_new = static_ok_u (dlist_old, action, window);
else if (strcmp (name, "V") == 0)
  dlist_new = static_ok_v (dlist_old, action, window);
else if (strcmp (name, "W") == 0)
  dlist_new = static_ok_w (dlist_old, action, window);
...
if (dlist_old != static_before && dlist_old != gamgi->directions)
  gamgi_engine_dlist_remove_all (dlist_old);

gamgi_engine_dlist_remove_all (static_before);
static_before = gamgi->directions;

if (scope == STATIC_CURRENT)
  gamgi->directions = dlist_new;
else
  gamgi_gtk_select_merge (&gamgi->directions, dlist_new);

/**********************************************
 * Change color for Direction label to signal *
 * that a list of selected directions exists  *
 *                                            *
 * reset dialog: the Scope list is set        *
 * to Current and the model is updated        *
 **********************************************/

gamgi_global_selection_update_windows (&gamgi->directions, "button_direction");
...
static_before = NULL;
gamgi_global_selection_unlink (gamgi->directions);
...
if (row == 0)
  ...
  if ((event->state & GDK_CONTROL_MASK) == 0)
    ...
    if (window_dialog == window_mouse)
      ...
      x, y, GAMGI_ENGINE_DIRECTION, FALSE, NULL);
    }
   ...
   *********************************************************/
    ...
    gamgi_mesa_select_object (window_mouse,
    x, y, GAMGI_ENGINE_DIRECTION, TRUE, static_press);
  }
...
dialog = gamgi_gtk_dialog_task0_create ("Direction Select", window);
window->action = GAMGI_GTK_DIRECTION_SELECT;
...
gtk_tree_store_append (tree_store, &child, &iter);
gtk_tree_store_set (tree_store, &child, 0, "U", -1);
gtk_tree_store_append (tree_store, &child, &iter);
gtk_tree_store_set (tree_store, &child, 0, "V", -1);
gtk_tree_store_append (tree_store, &child, &iter);
gtk_tree_store_set (tree_store, &child, 0, "W", -1);
