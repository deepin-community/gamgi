/***********************************************
 *
 * $GAMGI/src/gtk/plane/gamgi_gtk_plane_copy.c
 *
...
/***********************************************
 * initialize plane, name of copies, number    *
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT plane);
else
  layer = cache.layer;
...
for (i = 0; i < cache.number; i++)
  ...
  if (cache.name[0] != '\0')
    strcpy (plane_new->object.name, cache.name);

  gamgi_engine_link_plane_layer (plane_new, cache.layer);

  gamgi_math_vector_add (translate, cache.shift, translate);
  gamgi_mesa_translate_plane (plane_new, translate);
  }
...
/****************************************
 * copy planes according to dialog data *
 ****************************************/

start = NULL;
for (dlist = gamgi->planes; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_PLANE dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *   any plane object can be used here   *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->planes->data;
...
/***************************************
 * copy plane according to dialog data *
 ***************************************/

static_apply (plane, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT plane,
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_PLANE_COPY_PRESS        *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_plane_copy_press (gamgi_window *window_mouse,  
...
/*************** external function *************
 *                                             *
 *              GAMGI_GTK_PLANE_COPY           *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_plane_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Plane Copy", window);
window->action = GAMGI_GTK_PLANE_COPY;
