/***********************************************
 *
 * $GAMGI/src/gtk/plane/gamgi_gtk_plane_link.c
 *
...
/********************************************
 * when entries are empty, look for a plane *
 ********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_PLANE;
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  row = gtk_combo_box_get_active (GTK_COMBO_BOX (combo));
  if (row == 0) return GAMGI_ENGINE_DIRECTION;
  if (row == 1) return GAMGI_ENGINE_ATOM;
  if (row == 2) return GAMGI_ENGINE_ORBITAL;
  if (row == 3) return GAMGI_ENGINE_TEXT;
  }
 ...
 *                     STATIC_RESET                   *
 *                                                    *
 * Reset the plane link dialog, cleaning all entries. *
 *                                                    *
 ******************************************************/
 ...
 *                 STATIC_LIST           *
 *                                       *
 * Makes the plane entry sensitive when  *
...
{
if (child->class == GAMGI_ENGINE_PLANE &&
(GAMGI_CAST_PLANE child)->model != GAMGI_PHYS_POLYGON)
  {
  if (cache.local == TRUE)
...
if (gamgi->planes == NULL)
  { gamgi_gtk_dialog_message_create ("Error",
  "Empty list of plane objects", window_dialog); return; }
...
if (cache.above == TRUE)
  {
  /*******************************************
   * link above: link each listed plane      *
  ...
  {
  /**************************************************
   * link below: link each listed plane (parent) to *
...
/*******************************
 * pick plane, get dialog data *
 *******************************/

plane = GAMGI_CAST_PLANE gamgi_gtk_object_name_number (dialog,
"entry_plane", GAMGI_ENGINE_PLANE, window_dialog);
if (plane == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid plane object", window_dialog);
  return;
  }
...
/***********************************************************************
 * link above: link selected plane (child) to selected object (parent) *
 * link below: link selected plane (parent) to selected object (child) *
 ***********************************************************************/

if (cache.above == TRUE)
  { child = GAMGI_CAST_OBJECT plane; parent = cache.object; }
else
  { child = cache.object; parent = GAMGI_CAST_OBJECT plane; }
...
dialog = gamgi_gtk_dialog_task0_create ("Plane Link", window);
window->action = GAMGI_GTK_PLANE_LINK;
