/*********************************************
 *
 * $GAMGI/src/gtk/atom/gamgi_gtk_atom_copy.c
 *
...
/*******************************************
 * when entries are empty, look for a atom *
 *******************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_ATOM;
...
/***********************************************
 * initialize atom, name of copies, number     *
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT atom);
else
  layer = cache.layer;
 ...
 *                                                 *
 *  copy atoms and their contents, cleaning the    *
 *  bonds formed with atoms that are not copied    *
 *                                                 *
 *  remove list of atoms used to clean the bonds   *
 ***************************************************/

atom_start = gamgi_engine_dlist_atom_object (GAMGI_CAST_OBJECT atom, NULL);
...
for (i = 0; i < cache.number; i++)
  ...
  if (cache.name[0] != '\0')
    strcpy (atom_new->object.name, cache.name);

  gamgi_engine_link_atom_layer (atom_new, layer);
  ...
  gamgi_mesa_translate_atom (atom_new, translate);
  }
...
if (gamgi->atoms == NULL)
  ...
  "Empty list of atom objects", window_dialog); return; }
...
/***************************************
 * copy atoms according to dialog data *
 ***************************************/

start = NULL;
for (dlist = gamgi->atoms; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_ATOM dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *   any atom object can be used here    *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->atoms->data;
...
"entry_atom", GAMGI_ENGINE_ATOM, window_dialog);
if (atom == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid atom object", window_dialog);
  return;
  }
...
/**************************************
 * copy atom according to dialog data *
 **************************************/

static_apply (atom, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT atom,
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_ATOM_COPY_PRESS         *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_atom_copy_press (gamgi_window *window_mouse,  
...
void gamgi_gtk_atom_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Atom Copy", window);
window->action = GAMGI_GTK_ATOM_COPY;
