/***************************************************
 *
 * $GAMGI/src/gtk/window/gamgi_gtk_window_select.c
 *
...
enum { STATIC_GAMGI, STATIC_BEFORE, STATIC_CURRENT };
 ...
 * available only while the Window->Select dialog exists.  *
 ***********************************************************/
...
{
/*************************************************
 * everytime the dialog is removed: 1) free list *
 * Before; 2) link objects in list Current.      *
 *************************************************/

gamgi_engine_dlist_remove_all (static_before);
gamgi_global_selection_link (gamgi->windows);
}
...
while (dlist != NULL)
  {
  gamgi_gtk_select_window (store, NULL, GAMGI_CAST_WINDOW dlist->data);
  dlist = dlist->next;
  }
...
switch (scope)
  {
  case STATIC_GAMGI:
  dlist = gamgi_engine_dlist_window_gamgi (NULL);
  break;
...
static_model (gamgi->windows, window);
...
/**************************************************************
 * build list of candidate windows, according to chosen scope *
 *                                                            *
 * build list of selected windows, according to chosen method *
 **************************************************************/
...
if (dlist_old != static_before && dlist_old != gamgi->windows)
  gamgi_engine_dlist_remove_all (dlist_old);

gamgi_engine_dlist_remove_all (static_before);
static_before = gamgi->windows;

if (scope == STATIC_CURRENT)
  gamgi->windows = dlist_new;
else
  gamgi_gtk_select_merge (&gamgi->windows, dlist_new);

/*******************************************
 * Change color for Window label to signal *
 * that a list of selected windows exists  *
 *                                         *
...
static_before = NULL;
gamgi_global_selection_unlink (gamgi->windows);
...
if (gamgi->windows == NULL)
  gtk_combo_box_set_active (GTK_COMBO_BOX (combo), STATIC_GAMGI);
else
  gtk_combo_box_set_active (GTK_COMBO_BOX (combo), STATIC_CURRENT);
...
if (row == 0)
  ...
  if ((event->state & GDK_CONTROL_MASK) == 0)
    ...
    if (window_dialog == window_mouse)
      ...
      x, y, GAMGI_ENGINE_WINDOW, FALSE, NULL);
    }
   ...
   *********************************************************/
    ...
    gamgi_mesa_select_object (window_mouse,
    x, y, GAMGI_ENGINE_WINDOW, TRUE, static_press);
  }
...
dialog = gamgi_gtk_dialog_task0_create ("Window Select", window);
window->action = GAMGI_GTK_WINDOW_SELECT;
