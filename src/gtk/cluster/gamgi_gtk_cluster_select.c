/*****************************************************
 *
 * $GAMGI/src/gtk/cluster/gamgi_gtk_cluster_select.c
 *
 ...
 * available only while the Cluster->Select dialog exists. *
 ***********************************************************/
...
gamgi_global_selection_link (gamgi->clusters);
...
while (dlist != NULL)
  {
  gamgi_gtk_select_cluster (store, NULL, GAMGI_CAST_CLUSTER dlist->data);
  dlist = dlist->next;
  }
...
switch (scope)
  {
  case STATIC_GAMGI:
  dlist = gamgi_engine_dlist_cluster_gamgi (NULL);
  break;

  case STATIC_WINDOW:
  dlist = gamgi_engine_dlist_cluster_window (window, NULL);
  break;

  case STATIC_LAYER:
  dlist = gamgi_engine_dlist_cluster_layer (window->layer, NULL);
  break;
...
static_model (gamgi->clusters, window);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  condition = (cluster->object.scale >= min && cluster->object.scale <= max);

  if (condition != action)
    {
    dlist_new = gamgi_engine_dlist_add_start (dlist_new);
    dlist_new->data = cluster;
    }
  }
...
/****************************************************************
 * build list of candidate clusters, according to chosen scope *
 *                                                              *
 * build list of selected clusters, according to chosen method *
 ****************************************************************/
...
if (dlist_old != static_before && dlist_old != gamgi->clusters)
  gamgi_engine_dlist_remove_all (dlist_old);

gamgi_engine_dlist_remove_all (static_before);
static_before = gamgi->clusters;

if (scope == STATIC_CURRENT)
  gamgi->clusters = dlist_new;
else
  gamgi_gtk_select_merge (&gamgi->clusters, dlist_new);

/********************************************
 * Change color for Cluster label to signal *
 * that a list of selected clusters exists  *
 *                                          *
 * reset dialog: the Scope list is set      *
 * to Current and the model is updated      *
 ********************************************/

gamgi_global_selection_update_windows (&gamgi->clusters, "button_cluster");
...
static_before = NULL;
gamgi_global_selection_unlink (gamgi->clusters);
...
void gamgi_gtk_cluster_select_press (gamgi_window *window_mouse,    
...
if (row == 0)
  ...
  if ((event->state & GDK_CONTROL_MASK) == 0)
    ...
    if (window_dialog == window_mouse)
      ...
      x, y, GAMGI_ENGINE_CLUSTER, FALSE, NULL);
    }
   ...
   *********************************************************/
    ...
    gamgi_mesa_select_object (window_mouse,
    x, y, GAMGI_ENGINE_CLUSTER, TRUE, static_press);
  }
...
void gamgi_gtk_cluster_select (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Cluster Select", window);
window->action = GAMGI_GTK_CLUSTER_SELECT;
