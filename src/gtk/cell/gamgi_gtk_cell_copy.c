/*********************************************
 *
 * $GAMGI/src/gtk/cell/gamgi_gtk_cell_copy.c
 *
...
/***********************************************
 * initialize cell, name of copies, number     *
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT cell);
else
  layer = cache.layer;
...
for (i = 0; i < cache.number; i++)
  {
  cell_new = gamgi_engine_copy_cell (cell);
  if (cache.name[0] != '\0')
    strcpy (cell_new->object.name, cache.name);

  gamgi_engine_link_cell_layer (cell_new, cache.layer);

  gamgi_math_vector_add (translate, cache.shift, translate);
  gamgi_mesa_translate_cell (cell_new, translate);
  }
...
if (gamgi->cells == NULL)
  ...
  "Empty list of cell objects", window_dialog); return; }
...
/***************************************
 * copy cells according to dialog data *
 ***************************************/

start = NULL;
for (dlist = gamgi->cells; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_CELL dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *    any cell object can be used here   *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->cells->data;
...
"entry_cell", GAMGI_ENGINE_CELL, window_dialog);
if (cell == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid cell object", window_dialog);
  return;
  }
...
/**************************************
 * copy cell according to dialog data *
 **************************************/

static_apply (cell, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT cell,
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_CELL_COPY_PRESS         *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_cell_copy_press (gamgi_window *window_mouse,  
...
void gamgi_gtk_cell_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Cell Copy", window);
window->action = GAMGI_GTK_CELL_COPY;
