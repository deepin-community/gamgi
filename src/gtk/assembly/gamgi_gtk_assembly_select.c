/*******************************************************
 *
 * $GAMGI/src/gtk/assembly/gamgi_gtk_assembly_select.c
 *
 ...
 * available only while the Assembly->Select dialog exists. *
 ************************************************************/
...
gamgi_global_selection_link (gamgi->assemblys);
...
while (dlist != NULL)
  {
  gamgi_gtk_select_assembly (store, NULL, GAMGI_CAST_ASSEMBLY dlist->data);
  dlist = dlist->next;
  }
...
switch (scope)
  {
  case STATIC_GAMGI:
  dlist = gamgi_engine_dlist_assembly_gamgi (NULL);
  break;

  case STATIC_WINDOW:
  dlist = gamgi_engine_dlist_assembly_window (window, NULL);
  break;

  case STATIC_LAYER:
  dlist = gamgi_engine_dlist_assembly_layer (window->layer, NULL);
  break;
...
static_model (gamgi->assemblys, window);
...
/**********************************
 * get list of selected assemblys *
 **********************************/
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  condition = (assembly->object.scale >= min && assembly->object.scale <= max);

  if (condition != action)
    {
    dlist_new = gamgi_engine_dlist_add_start (dlist_new);
    dlist_new->data = assembly;
    }
  }
...
/****************************************************************
 * build list of candidate assemblys, according to chosen scope *
 *                                                              *
 * build list of selected assemblys, according to chosen method *
 ****************************************************************/
...
if (dlist_old != static_before && dlist_old != gamgi->assemblys)
  gamgi_engine_dlist_remove_all (dlist_old);

gamgi_engine_dlist_remove_all (static_before);
static_before = gamgi->assemblys;

if (scope == STATIC_CURRENT)
  gamgi->assemblys = dlist_new;
else
  gamgi_gtk_select_merge (&gamgi->assemblys, dlist_new);

/*********************************************
 * Change color for Assembly label to signal *
 * that a list of selected assemblys exists  *
 *                                           *
 * reset dialog: the Scope list is set       *
 * to Current and the model is updated       *
 *********************************************/

gamgi_global_selection_update_windows (&gamgi->assemblys, "button_assembly");
...
static_before = NULL;
gamgi_global_selection_unlink (gamgi->assemblys);
...
void gamgi_gtk_assembly_select_press (gamgi_window *window_mouse,    
...
if (row == 0)
  ...
  if ((event->state & GDK_CONTROL_MASK) == 0)
    ...
    if (window_dialog == window_mouse)
      ...
      x, y, GAMGI_ENGINE_ASSEMBLY, FALSE, NULL);
    }
   ...
   *********************************************************/
    ...
    gamgi_mesa_select_object (window_mouse,
    x, y, GAMGI_ENGINE_ASSEMBLY, TRUE, static_press);
  }
...
void gamgi_gtk_assembly_select (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Assembly Select", window);
window->action = GAMGI_GTK_ASSEMBLY_SELECT;
