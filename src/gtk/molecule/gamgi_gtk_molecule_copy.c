/*****************************************************
 *
 * $GAMGI/src/gtk/molecule/gamgi_gtk_molecule_copy.c
 *
...
/***********************************************
 * when entries are empty, look for a molecule *
 ***********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_MOLECULE;
...
/***********************************************
 * initialize molecule, name of copies, number *
...
static gamgi_bool static_apply (gamgi_molecule *molecule, gamgi_window *window)
{
gamgi_layer *layer;
gamgi_molecule *molecule_new = NULL;
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT molecule);
else
  layer = cache.layer;
 ...
 *                                                 *
 * copy clusters and their contents, cleaning the  *
 * bonds formed with atoms that are not copied     *
 *                                                 *
 *  remove list of atoms used to clean the bonds   *
 ***************************************************/

atom_start = gamgi_engine_dlist_atom_object (GAMGI_CAST_OBJECT molecule, NULL);
...
for (i = 0; i < cache.number; i++)
  {
  molecule_new = gamgi_engine_copy_molecule (molecule);
  if (cache.name[0] != '\0')
    strcpy (molecule_new->object.name, cache.name);

  gamgi_engine_link_molecule_layer (molecule_new, cache.layer);
  ...
  gamgi_mesa_translate_molecule (molecule_new, translate);
  }
...
if (gamgi->molecules == NULL)
  ...
  "Empty list of molecule objects", window_dialog); return; }
...
/*******************************************
 * copy molecules according to dialog data *
 *******************************************/

start = NULL;
for (dlist = gamgi->molecules; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_MOLECULE dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *  any molecule object can be used here *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->molecules->data;
...
"entry_molecule", GAMGI_ENGINE_MOLECULE, window_dialog);
if (molecule == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid molecule object", window_dialog);
  return;
  }
...
/******************************************
 * copy molecule according to dialog data *
 ******************************************/

static_apply (molecule, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT molecule,
...
/*************** external function *************
 *                                             *
 *         GAMGI_GTK_MOLECULE_COPY_PRESS       *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_molecule_copy_press (gamgi_window *window_mouse,  
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_MOLECULE_COPY           *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_molecule_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Molecule Copy", window);
window->action = GAMGI_GTK_MOLECULE_COPY;
