/***********************************************
 *
 * $GAMGI/src/gtk/light/gamgi_gtk_light_copy.c
 *
...
/********************************************
 * when entries are empty, look for a light *
 ********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_LIGHT;
...
/***********************************************
 * initialize light, name of copies, number    *
...
if (cache.layer == NULL)
  layer = GAMGI_CAST_LAYER light->object.object;
else
  layer = cache.layer;
...
if (layer->light + cache.number > gamgi->max_lights)
  {
  if (cache.local == TRUE)
...
layer->light += cache.number;

gamgi_math_vector_zero (translate);
for (i = 0; i < cache.number; i++)
  {
  light_new = gamgi_engine_copy_light (light);
  if (cache.name[0] != '\0')
    strcpy (light_new->object.name, cache.name);

  gamgi_engine_link_light_layer (light_new, layer);

  gamgi_math_vector_add (translate, cache.shift, translate);
  gamgi_mesa_translate_light (light_new, translate);
  }
...
if (gamgi->lights == NULL)
  ...
  "Empty list of light objects", window_dialog); return; }
...
/****************************************
 * copy lights according to dialog data *
 ****************************************/

start = NULL;
for (dlist = gamgi->lights; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_LIGHT dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   *   any light object can be used here   *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->lights->data;
...
"entry_light", GAMGI_ENGINE_LIGHT, window_dialog);
if (light == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid light object", window_dialog);
  return;
  }
...
/**************************************
 * copy light according to dialog data *
 **************************************/

if (static_apply (light, window_dialog) == FALSE) return;
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT light);
else
  layer = cache.layer;
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT light,
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_LIGHT_COPY_PRESS        *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_light_copy_press (gamgi_window *window_mouse,  
...
/*************** external function *************
 *                                             *
 *             GAMGI_GTK_LIGHT_COPY            *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_light_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Light Copy", window);
window->action = GAMGI_GTK_LIGHT_COPY;
