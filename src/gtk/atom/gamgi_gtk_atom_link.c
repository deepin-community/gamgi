/*********************************************
 *
 * $GAMGI/src/gtk/atom/gamgi_gtk_atom_link.c
 *
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  row = gtk_combo_box_get_active (GTK_COMBO_BOX (combo));
  if (row == 0) return GAMGI_ENGINE_ORBITAL;
  if (row == 1) return GAMGI_ENGINE_TEXT;
  }
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  gtk_combo_box_set_active (GTK_COMBO_BOX (combo_below), 1);
  }
 ...
 *                    STATIC_RESET                   *
 *                                                   *
 * Reset the atom link dialog, cleaning all entries. *
 *                                                   *
 *****************************************************/
 ...
 * as the atom might be useful for next task *
 *********************************************/
 ...
 *                 STATIC_LIST           *
 *                                       *
 * Makes the atom entry sensitive when   *
 * the list button is up and insensitive *
 * when the list button is down.         *
 *                                       *
 *****************************************/
...
if (cache.above == TRUE)
  {
  /*******************************************
   * link above: link each listed atom       *
  ...
  {
  /**************************************************
   * link below: link each listed atom (parent) to  *
...
/**********************************************************************
 * link above: link selected atom (child) to selected object (parent) *
 * link below: link selected atom (parent) to selected object (child) *
 **********************************************************************/

if (cache.above == TRUE)
  { child = GAMGI_CAST_OBJECT atom; parent = cache.object; }
else
  { child = cache.object; parent = GAMGI_CAST_OBJECT atom; }
...
/**************** external function ****************
 *                                                 *
 *            GAMGI_GTK_ATOM_LINK_PRESS            *
 *                                                 *
 ***************************************************/

void gamgi_gtk_atom_link_press (gamgi_window *window_mouse,
...
void gamgi_gtk_atom_link (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Atom Link", window);
window->action = GAMGI_GTK_ATOM_LINK;
