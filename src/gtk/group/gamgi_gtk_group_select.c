/*************************************************
 *
 * $GAMGI/src/gtk/group/gamgi_gtk_group_select.c
 *
 ...
 * available only while the Group->Select dialog exists.   *
 ***********************************************************/
...
gamgi_global_selection_link (gamgi->groups);
...
while (dlist != NULL)
  {
  gamgi_gtk_select_group (store, NULL, GAMGI_CAST_GROUP dlist->data);
  dlist = dlist->next;
  }
...
switch (scope)
  {
  case STATIC_GAMGI:
  dlist = gamgi_engine_dlist_group_gamgi (NULL);
  break;

  case STATIC_WINDOW:
  dlist = gamgi_engine_dlist_group_window (window, NULL);
  break;

  case STATIC_LAYER:
  dlist = gamgi_engine_dlist_group_layer (window->layer, NULL);
  break;
...
static void static_method_autonomy (char *label, gamgi_window *window)
{
GtkWidget *dialog = window->dialog0;
GtkWidget *widget;
GtkWidget *hbox;
...
else if (strcmp (name, "Autonomy") == 0)
  {
  static_method_autonomy ("widget_min", window);
  static_method_autonomy ("widget_max", window);
  }
...
static_model (gamgi->groups, window);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  ...
  condition = (group->red >= min && group->red <= max);

  if (condition != action)
    {
    dlist_new = gamgi_engine_dlist_add_start (dlist_new);
    dlist_new->data = group;
    }
  }
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  group = GAMGI_CAST_GROUP dlist->data;
  condition = (group->green >= min && group->green <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  group = GAMGI_CAST_GROUP dlist->data;
  condition = (group->blue >= min && group->blue <= max);
...
for (dlist = dlist_old; dlist != NULL; dlist = dlist->next)
  {
  group = GAMGI_CAST_GROUP dlist->data;
  condition = (group->object.scale >= min && group->object.scale <= max);
...
/****************************************************************
 * build list of candidate groups, according to chosen scope *
 *                                                              *
 * build list of selected groups, according to chosen method *
 ****************************************************************/
...
if (dlist_old != static_before && dlist_old != gamgi->groups)
  gamgi_engine_dlist_remove_all (dlist_old);

gamgi_engine_dlist_remove_all (static_before);
static_before = gamgi->groups;

if (scope == STATIC_CURRENT)
  gamgi->groups = dlist_new;
else
  gamgi_gtk_select_merge (&gamgi->groups, dlist_new);

/******************************************
 * Change color for Group label to signal *
 * that a list of selected groups exists  *
 *                                        *
 * reset dialog: the Scope list is set    *
 * to Current and the model is updated    *
 ******************************************/

gamgi_global_selection_update_windows (&gamgi->groups, "button_group");
...
static_before = NULL;
gamgi_global_selection_unlink (gamgi->groups);
...
if (row == 0)
  ...
  if ((event->state & GDK_CONTROL_MASK) == 0)
    ...
    if (window_dialog == window_mouse)
      ...
      x, y, GAMGI_ENGINE_GROUP, FALSE, NULL);
    }
   ...
   *********************************************************/
    ...
    gamgi_mesa_select_object (window_mouse,
    x, y, GAMGI_ENGINE_GROUP, TRUE, static_press);
  }
...
dialog = gamgi_gtk_dialog_task0_create ("Group Select", window);
window->action = GAMGI_GTK_GROUP_SELECT;
...
gtk_tree_store_append (tree_store, &child, &iter);
gtk_tree_store_set (tree_store, &child, 0, "Central", -1);
gtk_tree_store_append (tree_store, &child, &iter);
gtk_tree_store_set (tree_store, &child, 0, "Neighbour", -1);
gtk_tree_store_append (tree_store, &child, &iter);
