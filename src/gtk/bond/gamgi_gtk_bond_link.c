/*********************************************
 *
 * $GAMGI/src/gtk/bond/gamgi_gtk_bond_link.c
 *
...
/*******************************************
 * when entries are empty, look for a bond *
 *******************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_BOND;
 ...
 *                    STATIC_RESET                   *
 *                                                   *
 * Reset the bond link dialog, cleaning all entries. *
 *                                                   *
 *****************************************************/
 ...
 *                STATIC_LIST              *
 *                                         *
 * Makes the bond entry sensitive when     *
...
if (gamgi->bonds == NULL)
  { gamgi_gtk_dialog_message_create ("Error",
  "Empty list of bond objects", window_dialog); return; }
...
/******************************************
 * link each listed bond to a child       *
 * object copied from the selected object *
 ******************************************/
...
/******************************
 * pick bond, get dialog data *
 ******************************/

bond = GAMGI_CAST_BOND gamgi_gtk_object_name_number (dialog,
"entry_bond", GAMGI_ENGINE_BOND, window_dialog);
if (bond == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid bond object", window_dialog);
  return;
  }

if (static_grab (window_dialog) == FALSE) return;

child = cache.object; parent = GAMGI_CAST_OBJECT bond;
...
/**************** external function ****************
 *                                                 *
 *            GAMGI_GTK_BOND_LINK_PRESS            *
 *                                                 *
 ***************************************************/

void gamgi_gtk_bond_link_press (gamgi_window *window_mouse,
 ...
 *                GAMGI_GTK_BOND_LINK              *
 *                                                 *
 * Creates the dialog window used to create bonds. *
 *                                                 *
 ***************************************************/

void gamgi_gtk_bond_link (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Bond Link", window);
window->action = GAMGI_GTK_BOND_LINK;
