/***********************************************
 *
 * $GAMGI/src/gtk/group/gamgi_gtk_group_copy.c
 *
...
/********************************************
 * when entries are empty, look for a group *
 ********************************************/

if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == FALSE &&
gamgi_io_token_check (name) == FALSE) return GAMGI_ENGINE_GROUP;
...
/***********************************************
 * initialize group, name of copies, number *
...
if (cache.layer == NULL)
  layer = gamgi_engine_find_layer (GAMGI_CAST_OBJECT group);
else
  layer = cache.layer;
 ...
 *                                                 *
 * copy groups and their contents, cleaning the    *
 * bonds formed with atoms that are not copied     *
 *                                                 *
 *  remove list of atoms used to clean the bonds   *
 ***************************************************/

atom_start = gamgi_engine_dlist_atom_object (GAMGI_CAST_OBJECT group, NULL);
...
for (i = 0; i < cache.number; i++)
  {
  group_new = gamgi_engine_copy_group (group);
  if (cache.name[0] != '\0')
    strcpy (group_new->object.name, cache.name);

  gamgi_engine_link_group_layer (group_new, layer);
  ...
  gamgi_mesa_translate_group (group_new, translate);
  }
...
if (gamgi->groups == NULL)
  ...
  "Empty list of group objects", window_dialog); return; }
...
/****************************************
 * copy groups according to dialog data *
 ****************************************/

start = NULL;
for (dlist = gamgi->groups; dlist != NULL; dlist = dlist->next)
  if (static_apply (GAMGI_CAST_GROUP dlist->data, window_dialog) == FALSE)
    {
...
if (layer == NULL)
   ...
   *                                       *
   * any group object can be used here     *
   *****************************************/

  object = GAMGI_CAST_OBJECT gamgi->groups->data;
...
"entry_group", GAMGI_ENGINE_GROUP, window_dialog);
if (group == NULL)
  {
  gamgi_gtk_dialog_message_create ("Error",
  "Invalid group object", window_dialog);
  return;
  }
...
/***************************************
 * copy group according to dialog data *
 ***************************************/

static_apply (group, window_dialog);
...
gamgi_gtk_object_copy (GAMGI_CAST_OBJECT group,
...
/*************** external function *************
 *                                             *
 *           GAMGI_GTK_GROUP_COPY_PRESS        *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_group_copy_press (gamgi_window *window_mouse,  
...
/*************** external function *************
 *                                             *
 *             GAMGI_GTK_GROUP_COPY            *
 *                                             *
 *                                             *
 ***********************************************/

void gamgi_gtk_group_copy (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Group Copy", window);
window->action = GAMGI_GTK_GROUP_COPY;
