/*****************************************************
 *
 * $GAMGI/src/gtk/molecule/gamgi_gtk_molecule_link.c
 *
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  row = gtk_combo_box_get_active (GTK_COMBO_BOX (combo));
  if (row == 0) return GAMGI_ENGINE_GROUP;
  if (row == 1) return GAMGI_ENGINE_PLANE;
  if (row == 2) return GAMGI_ENGINE_DIRECTION;
  if (row == 3) return GAMGI_ENGINE_ATOM;
  if (row == 4) return GAMGI_ENGINE_ORBITAL;
  if (row == 5) return GAMGI_ENGINE_TEXT;
  }
...
if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)) == TRUE)
  ...
  gtk_combo_box_set_active (GTK_COMBO_BOX (combo_below), 3);
  }
 ...
 *                        STATIC_RESET                   *
 *                                                       *
 * Reset the molecule link dialog, cleaning all entries. *
 *                                                       *
 *********************************************************/
 ...
 *                  STATIC_LIST                *
 *                                             *
 * Makes the molecule entry sensitive when     *
...
if (cache.above == TRUE)
  {
  /*******************************************
   * link above: link each listed molecule   *
  ...
  {
  /*****************************************************
   * link below: link each listed molecule (parent) to *
...
/**************************************************************************
 * link above: link selected molecule (child) to selected object (parent) *
 * link below: link selected molecule (parent) to selected object (child) *
 **************************************************************************/

if (cache.above == TRUE)
  { child = GAMGI_CAST_OBJECT molecule; parent = cache.object; }
else
  { child = cache.object; parent = GAMGI_CAST_OBJECT molecule; }
...
/***************** external function ****************
 *                                                  *
 *           GAMGI_GTK_MOLECULE_LINK_PRESS          *
 *                                                  *
 ****************************************************/

void gamgi_gtk_molecule_link_press (gamgi_window *window_mouse,
...
/****************** external function *****************
 *                                                    *
 *               GAMGI_GTK_MOLECULE_LINK              *
 *                                                    *
 *  Creates the dialog window used to link molecules. *
 *                                                    *
 ******************************************************/

void gamgi_gtk_molecule_link (GtkWidget *widget, void *data)
{
gamgi_window *window = GAMGI_CAST_WINDOW data;
GtkWidget *dialog;
GtkWidget *notebook;
...
dialog = gamgi_gtk_dialog_task0_create ("Molecule Link", window);
window->action = GAMGI_GTK_MOLECULE_LINK;
